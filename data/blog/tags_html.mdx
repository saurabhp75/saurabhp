...
- `div`: `<div>` is a generic tag that can be used to group elements.
- `ul`: Unordered list.
- `ol`: Ordered list.
- `li`: List item.
- `nav`: Navigation bar.
...
`Note`: Most elements are block level elements by default.

## Block level elements feature

- Width of 100% of their parents.
- They should have height of 0 for responsive design.
...
margin: 2px 1em auto; /* top | right | bottom | left */
...
- Horizontal centering: `margin: 0, auto;` // Requires that the element width is specified.
