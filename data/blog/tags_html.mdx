...
- header, main, footer, div, nav, section.

Most elements are block level elements by default.

Block level elements have:

- Width of 100% of their parents.
- They have height of 0.
...
- It can have upto 4 attributes.
- 1 attribute: marging: all
- 4 attributes: margin: top, right, bottom, left
- 2 attributes: margin vertical, horizontal
- 3 attributes: margin: top, horizontal, bottom
...
- The totla width of the element also includes, padding, border and margin.

### Building responsive UI (CSS)

## Why we should never specify the height of a block level element:

When the width of a block level element is specified, it can grow in height to accomodate the content(text?).
But when height of a block element is specified the content can overflow the element, if the content is large.
Also the next element will not be spaced from the overflowing content but by the conatiner element itself.
...
- You should not set font-size using em,

```css
btn {
font-size: 2.5em; /_ 2.5 times of the font-size of the parent element._/
margin: 2em; /_ 2 times the font-size of current element, ie 5 times the size of parent element._/
}
```
...
/* Striped pattern */
background-image: linear-gradient(to left, red 10%, blue 10%, blue 20%, red 20%);
...
background-image: radial-gradient(to left, red 10%, blue 10%, blue 20%, red 20%);

/* Repeating circles */
backrgound-image: repeating-radial-gradient(red, red 10px, blue 10px, blue 20px);

/* Repeating lines */
backrgound-image: repeating-linear-gradient(red, red 10px, blue 10px, blue 20px);
```
...
## Why we use box-sizing property
...
- `inline-block` elements have width and height.
- `inline` elements have only margin-left, margin-right, padding-left, padding-right attributes.
- `inline-block` has margin, padding, height, width attributes.
...
- `align-content: start|center|space-between|space-around|stretch;` distribute content along cross axis, ie space between rows/columns, but only if we have multiple rows/columns and flex-wrap is enabled.
...
- `flex-grow`: Its an unitless number. defines the ability for a flex item to grow if necessary. Default is 0.
- `flex-basis 200px`: Defines the default/initial size of an element before additional space is distributed. The default is `auto`.
- `flex-shrink`: Its an unitless number. Defines the ability for a flex item to shrink if necessary. Default is 1.
