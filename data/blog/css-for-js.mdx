...
```css
/* Target the last paragraph child 
in a container */
p:last-child {
  margin-bottom: 0px;
}
...
- `::before/after`: These pseudo-elements are added inside the element, right before and after the element's content. `:: before and after` should not be used due to accessibility concerns.
...
\_ We can explicitly define the grid by specifying the number of columns and/or rows.

- `grid-template-columns`: Specify the columns.
