...
7. Difference between `.append()` and `appendChild()`: `.append` accepts Node
   objects and DOMStrings while `.appendChild` accepts only Node objects.
   `.append` does not have a return value while `.appendChild` returns the
   appended Node object. `.append` allows you to add multiple items while
